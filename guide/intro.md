...
The editor, settings page and media gallery are safety-disabled by default. Access can be enabled through the device manager `/admin/d.php` for single devices/browsers. PHP mode does not need `/admin/` for runtime; it needs `/admin/` only if image upload through the optional media gallery or device access tools are used. MD mode needs `/admin/` only when the built-in Markdown editor or media tools are used. If Markdown files are edited directly, for example via FTP, `/admin/` can be removed and MD runtime still rebuilds `/zpcache/`. Rich mode needs `/admin/`; without the Rich editor, the Rich mode concept is not useful. 
...
It shows the system status and links to Settings when `adminexposure >= 3`, and then to the device manager. The other tools are available from the top admin navigation. 
...
The default values in `/__config/EDITACCESS_ENABLE.php` are `return [0,0,0];`. To open access to the device manager for a short setup step, edit the values in that file, for example to `return [5,1,0];` or `return [5,1,1];`. The first number is the access time in minutes (capped at five minutes), the second number is the number of device cookies that may be created during this setup period, and the third number is the optional `adminexposure` level written into newly created cookies. When the third value is `0`, the new cookie uses the global config level later. After all allowed cookies are set, temporary access is immediately canceled.

If the second number is `0`, you can only view the device list and delete access cookies. - _On first access, the helper adds an internal UTC/GMT Unix expiry timestamp as a fourth value automatically. The timestamp is not extended by reloading. After the last allowed new device cookie, after deleting all access entries, or after expiry, the file is reset to `return [0,0,0];`._
...
The editor and gallery are not shown if the cookie data is missing, has no current access entry, or the matching entry has expired. The edit cookie data is stored in `/admin/inc/zdata.php`.
...
Cached templates use central placeholders. `~~ZBASEURL~~` is the explicit basepath placeholder. `~~ZDIRDESIGN~~` points to the selected design directory and is preferred for design CSS, JavaScript and images. `~~ZDIRSCRIPTS~~` points to `/layout/_scripts/`. The full placeholder list is in [`layout-generator-manual.md`]({{ '/guide/layout-generator-manual/' | relative_url }}#html-cache-standard).

## WordPress 2015 generator scripts

The WordPress variants (`layout/_zpgen/wordpress/gen2015.php` and `layout/_zpgen/wordpress/gen2026.php`) require the following local JavaScript files for compatibility with classic WordPress themes, which are listed in the `/layout/_scripts` directory:
...
**admin file**  
An administration file below `/admin/`, for example the editor, settings page, navigation editor, media gallery or update tools.
...
**Settings**  
The admin page for selecting the design and editing file-based `~~ZCOL2~~` content. The design select is available when `layout` is not set in `/__config/conf.php`. The sidebar field is available when `col2` is not set in `/__config/conf.php` and the active design supports `columns=2`.
...
ZANACMS can write a static `/sitemap.xml` from the current mode's sitemap buffer. The switch is enabled by preset. Disable it in `/__config/conf.php` with:

```php
'sitemap'=>false,
```

If `sitemap` is not set in `/__config/conf.php`, the admin Settings page can enable it as a fallback value. If it is set in `conf.php`, the Settings checkbox is hidden.

The current sitemap maintenance depends on the admin area: `/admin/maint.php` rebuilds the buffer through the admin maintenance pixel. The admin area must exist and admin pages must be used. If `/admin/` is removed or never opened, automatic sitemap rebuilding does not run.
