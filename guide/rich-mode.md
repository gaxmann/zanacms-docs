...
That file is a data fragment. It must not include `zp/zana.php` and it must not call `out_page()`. ZANACMS has already loaded the runtime before this fragment is included. The fragment may call `zautolg()` and then set `$GLOBALS['zdata']`. Example:
...
A normal PHP-mode page is different: it lives in the website root, includes `zp/zana.php`, prepares `$GLOBALS['zdata']`, and calls `out_page()`. A Rich PHP fragment only provides page data for the Rich page that includes it.
...
Rich mode sitemap rebuilding is available unless `sitemap` is disabled. If the key is missing, the preset is `true`. To disable it, set:

```php
'sitemap'=>false,
```

Normal Rich pages are read from `.data.php` page data. Rich pages with `mode=>'php'` read their sitemap-relevant values from the `/pages/PAGEID.php` fragment by including it directly; the fragment must not include `zp/zana.php` and must not call `out_page()`.
