...
`mode: php` loads `/pages/PAGEID.php` as a page data fragment while the website itself remains in MD mode. The PHP fragment must only set page data; it must not include `zp/zana.php` and must not call `out_page()`. ZANACMS has already loaded the runtime before the fragment is included. The fragment may call `zautolg()` and then set `$GLOBALS['zdata']`. A normal PHP-mode page is different: it lives in the website root, includes `zp/zana.php`, prepares `$GLOBALS['zdata']`, and calls `out_page()`.

For page `contact`, the MD frontmatter can use:
...
ZANACMS then includes `/pages/contact.php`. The fragment does not include `zp/zana.php` and does not call `out_page()`:
...
MD mode sitemap rebuilding is available unless `sitemap` is disabled. If the key is missing, the preset is `true`. To disable it, set:

```php
'sitemap'=>false,
```

Normal MD pages are read from their language-specific Markdown files and frontmatter. MD pages with `mode: php` read their sitemap-relevant values from the `/pages/PAGEID.php` fragment by including it directly; the fragment must not include `zp/zana.php` and must not call `out_page()`.
